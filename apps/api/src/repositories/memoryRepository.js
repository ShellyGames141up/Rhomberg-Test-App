...
export function createMemoryRepository(seed = {}) {
  ...
  return {
    ...
    async administerCompany(actor,companyId,operation,payload,correlationId) {
      ...
      if(operation==='delete') { if(String(payload.reason || '').trim().length < 8) { const error=new Error('Company deletion reason required.'); error.code='VALIDATION_ERROR'; error.statusCode=422; throw error; } company.deletedAt=new Date().toISOString(); company.status='archived'; for(const membership of state.users.filter(user=>user.companyIds?.includes(companyId))) membership.companyIds=membership.companyIds.filter(id=>id!==companyId); for(const representative of state.representatives) representative.companyIds=(representative.companyIds || []).filter(id=>id!==companyId); state.audits.push({id:state.audits.length+1,eventType:'administrator.company_changed',actorUserId:actor.id,actorRole:actor.role,companyId,action:'admin_delete',entityType:'company',entityId:companyId,outcome:'success',correlationId,details:{operation:'delete',reason:payload.reason},createdAt:new Date().toISOString()}); return clone({id:companyId,operation:'delete',status:'deleted'}); }
