...
export async function buildApp({ config, repository, storage, identityProvider, logger = true, logStream } = {}) {
  ...
  app.delete('/api/v1/administration/companies/:companyId', { preHandler: requireCsrf, schema: { params:{type:'object',required:['companyId'],properties:{companyId:{type:'string',format:'uuid'}}}, body:{type:'object',additionalProperties:false,required:['reason'],properties:{reason:{type:'string',minLength:8,maxLength:1000}}} } }, async request => ({ data: await administrationService.deleteCompany(request.actor,request.params.companyId,request.body || {},request.id), meta:{requestId:request.id} }));
