...
export function createAdministrationService({ repository, storage, identityProvider, passwordHasher = hashPassword }) {
  ...
  return Object.freeze({
    ...
    deleteCompany(actor,companyId,input,correlationId) { requirePermission(actor,'administer_users'); if(String(input?.reason || '').trim().length<8) throw validationError({reason:'Record why this customer company is being deleted.'}); return repository.administerCompany(actor,companyId,'delete',{reason:String(input.reason).trim()},correlationId); },
