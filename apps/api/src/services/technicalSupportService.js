...
export function createTechnicalSupportService({repository,storage}) {
  ...
  return Object.freeze({
    getByRfq:(actor,rfqId)=>repository.getTechnicalSupportByRfq(actor,rfqId),
    ...
    respond(actor,id,input,options) { requirePermission(actor,'respond_technical_support'); const response=clean(input.response),recommendation=clean(input.recommendation); if(response.length<10 || recommendation.length<5) throw validationError({response:'Enter a clear response and recommendation.'}); return this.postMessage(actor,id,{message:`${response}\n\nRecommendation: ${recommendation}`,classification:input.customerSafeNote?'customer_safe':'internal_only',...input},{...options,action:'technical_response',toStatus:'technical_response_submitted'}); },
