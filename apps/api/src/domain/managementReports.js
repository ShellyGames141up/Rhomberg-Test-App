// Shared renderer: callers must supply an authorised, scoped report projection.
...
const wrapText = (text, font, size, width) => {
  const words = cleanText(text).split(' ').filter(Boolean).flatMap(word => {
    ...
    for (const letter of word) {
      if (part && font.widthOfTextAtSize(part + letter, size) > width) { parts.push(part); part = ''; }
      part += letter;
    }
...
export const validateManagementReportOptions = (input = {}) => {
  ...
  return {
    periodMode: ['date_range', 'rolling_months'].includes(input.periodMode) ? input.periodMode : 'last_31_days',
...
export async function generateManagementPdfReport({ dashboard, options, generatedAt, generatedBy, roleLabel }) {
  ...
  const metrics = items => {
    for (let index = 0; index < items.length; index += 3) {
      ...
      row.forEach((item, column) => {
        ...
        const size = Math.min(15, 15 * 138 / Math.max(1, bold.widthOfTextAtSize(value, 15)));
        page.drawText(value, { x: x + 9, y: y - 18, font: bold, size, color: NAVY });
  ...
  const table = (headers, rows, widths) => {
    ...
    const inputWidth = widths.reduce((sum, value) => sum + value, 0);
    if (inputWidth > availableWidth) widths = widths.map(value => value * availableWidth / inputWidth);
    ...
    rows.forEach((row, rowIndex) => {
      ...
      while (offset < lineCount) {
        if (y - 28 < 65) { addPage(); drawHeader(); }
        const count = Math.min(lineCount - offset, Math.max(1, Math.floor((y - 73) / 10)));
        const rowHeight = Math.max(22, count * 10 + 8);
        if (rowIndex % 2 === 1) page.drawRectangle({ x: MARGIN, y: y - rowHeight + 3, width: totalWidth, height: rowHeight, color: PALE });
        let x = MARGIN;
        cells.forEach((lines, index) => {
          lines.slice(offset, offset + count).forEach((line, lineIndex) => page.drawText(line, { x: x + 5, y: y - 9 - lineIndex * 10, font: regular, size: 7.5, color: INK }));
          x += widths[index];
        });
        y -= rowHeight;
        offset += count;
  ...
  if (selected.has('quantity_demand')) {
    ...
    table(['Product', 'Units'], (dashboard.phase21?.products?.byProduct || []).map(row => [row.label, row.quantity]), [390, 101]);
  ...
  if (selected.has('operational_records')) {
    ...
    table(['Type', 'Reference', 'Customer', 'Representative', 'Status', 'Updated'], (dashboard.records || []).map(record => [record.workflowType, record.reference, record.company, record.selectedRep?.name || 'Unassigned', humanise(record.trackingStatus), dateLabel(record.updatedAt || record.createdAt)]), [45, 80, 115, 90, 95, 66]);
  }
