// Shared pure analytics used by the server and isolated public preview.
...
export const resolveManagementPeriod = (options = {}, now = new Date()) => {
  const mode = ['date_range', 'rolling_months'].includes(options.periodMode) ? options.periodMode : 'last_31_days';
  ...
  if (mode === 'date_range') {
    ...
    // Inclusive calendar-date window: today plus the previous thirty UTC dates.
...
const quoteCases = records => {
  ...
  for (const record of records) {
    ...
    cases.set(key, {
      ...
      quoteDate: isOrder && current.quoteDate && !record.quotation?.date ? current.quoteDate : quoteDateFor(record) || current.quoteDate,
