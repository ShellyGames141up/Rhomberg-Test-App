      ...
      {expanded && (
        <div className="expeditor-order-detail sales-rfq-detail">
          <div className="expeditor-facts">
            ...
            <span><small>Purchase Order</small><strong>{rfq.purchaseOrders?.find(item => item.active)?.number || 'Requested only after quotation acceptance'}</strong></span>
          ...
          {documents.length > 0 && <div className="sales-document-list"><strong>Internal Document Centre</strong>{documents.map(document => <span key={document.id}><b>{document.originalFilename || document.fileName || document.originalName}</b><small>{(document.category || document.documentType || 'document').replaceAll('_', ' ')} · {document.mimeType || document.mediaType || 'File'} · {Math.ceil(Number(document.fileSize || document.sizeBytes || 0) / 1024)} KB · {document.customerVisible ? 'Customer visible' : 'Internal only'}</small></span>)}</div>}
          {rfq.quotationVersions?.length > 0 && <div className="sales-document-list"><strong>Quotation version history</strong>{[...rfq.quotationVersions].sort((a, b) => b.versionNumber - a.versionNumber).map(version => <span key={version.id}><b>Version {version.versionNumber} · {version.quotationNumber}</b><small>{version.status.replaceAll('_', ' ')} · {version.isCurrent ? 'Current action version' : 'Preserved immutable version'}</small></span>)}</div>}
          {rfq.quotationRejections?.length > 0 && <div className="sales-document-list"><strong>Customer rejection history</strong>{[...rfq.quotationRejections].reverse().map(rejection => <span key={rejection.id}><b>{rejection.category.replaceAll('_', ' ')}</b><small>{rejection.explanation} · {formatDate(rejection.rejectedAt)}</small></span>)}</div>}
