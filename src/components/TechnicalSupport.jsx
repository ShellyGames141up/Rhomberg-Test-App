...
function MessageThread({ request, onPost, customer = false, allowPost = true }) {
  ...
  return <section className="technical-thread" aria-label="Technical correspondence" data-live-editing={Boolean(message || attachment || busy)}>
...
export function TechnicalSupportWorkspace({ account, actions, onChanged, focusRecordId = '', refreshToken = 0 }) {
  ...
  useEffect(() => { load(); }, [search, status, priority, refreshToken]);
