export function AppHeader({ account, onNavigate, onBack, backLabel }) {
  ...
  return (
    <header className="app-header">
      {onBack ? (
        ...
        <button className="mini-brand" type="button" onClick={() => onNavigate('home')} aria-label="Rhomberg home">
          <img src="assets/images/rhomberg-gauge-mark.svg" alt="" />
          <span><strong>RHOMBERG</strong><small>QUOTE PORTAL</small></span>
        </button>
      )}
      {onBack && <span className="header-context">{backLabel || 'Catalogue'}</span>}
      <div className="header-tools"><span className="preview-status"><i /> Preview</span><button className="header-avatar" type="button" onClick={() => onNavigate('account')} aria-label="Open account">{initials}</button></div>
    </header>
  );
}

export function BottomNav({ active, quantity, onNavigate }) {
  const items = [
    ['home', '⌂', 'Home'],
    ['catalogue', '▦', 'Catalogue'],
    ['enquiry', '+', 'Enquire'],
