...
export function WorkflowActionPanel({
  ...
  return (
    <div className="expeditor-update-box">
      ...
      {!['mark_quoted', 'accept_order', 'complete_planning', 'send_quotation', 'amend_quotation', 'close_rejected_rfq', 'verify_po_create_order'].includes(action.action) && !usesExpeditingFields(record, action.action) && !usesDispatchFields(record, action.action) && <label className="form-field"><span>Workflow comment {action.requiresComment ? <b>Required</b> : <i>Optional</i>}</span><textarea rows="3" value={note} onChange={event => setNote(event.target.value)} placeholder="Add a clear update for the audit history and customer timeline." />{fieldErrors.comment && <small className="field-error">{fieldErrors.comment}</small>}</label>}
...
function WorkflowActionFields({ action, data, onChange, errors = {}, record, account, planningOptions, expeditingOptions, dispatchOptions }) {
  ...
  if (['send_quotation', 'amend_quotation'].includes(action.action)) {
    return <div className="quotation-confirmation-fields">
      <p className="workflow-helper quote-workflow-helper"><strong>Immutable quotation version.</strong> PDF is preferred. Mock mode stores safe metadata only; production must validate and malware-scan the file server-side.</p>
      <div className="form-grid">
        <label className="form-field"><span>Quotation number <b>Required</b></span><input value={data.quotationNumber || ''} onChange={event => set('quotationNumber', event.target.value)} />{errors.quotationNumber && <small className="field-error">{errors.quotationNumber}</small>}</label>
        <label className="form-field"><span>Quotation date <b>Required</b></span><input type="date" value={data.quotationDate || ''} onChange={event => set('quotationDate', event.target.value)} />{errors.quotationDate && <small className="field-error">{errors.quotationDate}</small>}</label>
        <label className="form-field"><span>Expiry date <b>Required</b></span><input type="date" value={data.quotationExpiryDate || ''} onChange={event => set('quotationExpiryDate', event.target.value)} />{errors.quotationExpiryDate && <small className="field-error">{errors.quotationExpiryDate}</small>}</label>
        <label className="form-field quote-file-field"><span>Quotation attachment <b>Required</b></span><input type="file" accept=".pdf,.docx,.xlsx" onChange={event => set('documentFile', event.target.files?.[0] || null)} />{data.documentFile && <small>{data.documentFile.name} · {Math.ceil(data.documentFile.size / 1024)} KB</small>}{errors.document && <small className="field-error">{errors.document}</small>}</label>
      </div>
      <label className="form-field"><span>Customer-facing message <b>Required</b></span><textarea rows="3" value={data.customerMessage || ''} onChange={event => set('customerMessage', event.target.value)} />{errors.customerMessage && <small className="field-error">{errors.customerMessage}</small>}</label>
      <label className="form-field"><span>Internal note <i>Optional</i></span><textarea rows="3" value={data.internalNote || ''} onChange={event => set('internalNote', event.target.value)} /></label>
    </div>;
  }
  if (action.action === 'close_rejected_rfq') return <div className="order-acceptance-fields"><label className="form-field"><span>Closure reason <b>Required</b></span><textarea rows="4" value={data.closureReason || ''} onChange={event => set('closureReason', event.target.value)} />{errors.closureReason && <small className="field-error">{errors.closureReason}</small>}</label><label className="form-field"><span>Internal note <i>Optional</i></span><textarea value={data.internalNote || ''} onChange={event => set('internalNote', event.target.value)} /></label><label className="choice-row"><input type="checkbox" checked={Boolean(data.confirmed)} onChange={event => set('confirmed', event.target.checked)} /><span>Confirm that this rejected RFQ should be closed without deleting its history.</span></label></div>;
  if (action.action === 'verify_po_create_order') return <div className="order-acceptance-fields"><p className="workflow-helper"><strong>Accepted Quotations Awaiting Verification.</strong> Review the current quotation and customer PO before creating one idempotent Planning order.</p><label className="choice-row"><input type="checkbox" checked={Boolean(data.verified)} onChange={event => set('verified', event.target.checked)} /><span>I verified the Purchase Order against the accepted quotation version.</span></label><label className="form-field"><span>Internal note <i>Optional</i></span><textarea value={data.internalNote || ''} onChange={event => set('internalNote', event.target.value)} /></label></div>;
