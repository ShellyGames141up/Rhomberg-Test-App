...
export function Account({ account, enquiries, onSignOut, serviceMode, onOpenSettings, personalisation }) {
  ...
  return (
    <section className="app-screen account-screen" aria-labelledby="account-title">
      <div className="profile-hero"><span className="profile-avatar">{personalisation?.profileImage?.previewUrl ? <img src={personalisation.profileImage.previewUrl} alt="" style={{ objectPosition: `${personalisation.profileImage.position?.x || 50}% ${personalisation.profileImage.position?.y || 50}%` }} /> : initials}</span><span className="eyebrow">{isStaff ? 'Internal role access' : 'Company account'}</span><h1 id="account-title">{isStaff ? <>{roleLabel} <em>workspace</em></> : <>Your <em>workspace</em></>}</h1><strong>{account.company}</strong><small>{account.industry}</small></div>
      ...
      {!isStaff && onOpenSettings && <button className="account-settings-card" type="button" onClick={onOpenSettings}><span>AA</span><div><strong>Rhomberg Connect settings</strong><small>Theme, text size, spacing, images and notification preferences</small></div><i>›</i></button>}
