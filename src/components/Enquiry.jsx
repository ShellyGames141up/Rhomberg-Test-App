...
function EnquiryLine({ line, onEdit, onRemove, onQuantity }) {
  ...
  return (
    <article className="enquiry-line">
      ...
      <div className="enquiry-line-tools"><button type="button" className="edit-line" onClick={onEdit}>Edit configuration</button><div className="quantity-control-group"><span>Quantity</span><div className="line-quantity" role="group" aria-label={`${line.code} quantity controls`}><button type="button" onClick={() => onQuantity(normaliseQuantity(line.quantity - 1))} aria-label={`Decrease ${line.code} quantity`}>−</button><input type="number" min="1" max="9999" value={line.quantity} onChange={event => onQuantity(normaliseQuantity(event.target.value))} aria-label={`${line.code} quantity`} /><button type="button" onClick={() => onQuantity(normaliseQuantity(line.quantity + 1))} aria-label={`Increase ${line.code} quantity`}>+</button></div></div></div>
    </article>
  );
}
