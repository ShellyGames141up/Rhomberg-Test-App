...
const SECTION_META = Object.freeze({
  ...
  app: ['App Settings', 'Choose safe defaults without changing workflow rules.', 'A'],
  sounds: ['Sounds & Vibration', 'Control restrained interaction feedback for this device.', '♪'],
  notifications: ['Notifications', 'Choose relevant updates and simulated delivery channels.', '!'],
  ...
  security: ['Security & Sign-In', 'Review the session and use protected credential-change flows.', '◇'],
  tutorials: ['Help & Tutorials', 'Replay the complete customer journey or a focused feature guide.', '?'],
  privacy: ['Privacy and Data', 'Understand mock storage and production data boundaries.', 'i'],
  about: ['About Rhomberg Connect', 'Version, platform and demonstration limitations.', 'R'],
});
...
function TutorialSettings({ account, serviceMode, onReplay }) {
  ...
  return <div className="settings-stack"><SettingsPanel title="Guided tutorials">{customer ? <div className="tutorial-launch-grid">{TUTORIALS.map(item => <button type="button" className="secondary-button" key={item.id} onClick={() => onReplay(item.id)}>{item.label}</button>)}</div> : <p className="settings-note">Role-specific operational guidance is available from workspace help. The full customer RFQ tutorial is shown only to customer accounts.</p>}</SettingsPanel><SettingsPanel title="More help"><div className="settings-link-list"><button type="button">Frequently Asked Questions <small>Future approved content</small></button>{customer && <button type="button">Contact Assigned Representative <small>Use your authorised account contact</small></button>}<button type="button">Report a Problem <small>Future service desk integration</small></button><button type="button">View App Version <small>Rhomberg Connect 5.0.0</small></button><button type="button">Mock Mode Limitations <small>{serviceMode === 'mock' ? 'Fabricated data · same-device storage · simulated delivery' : 'Not applicable in production mode'}</small></button></div></SettingsPanel></div>;
}

function PrivacySettings({ serviceMode }) { return <SettingsPanel title="Privacy and data"><p>Customer and internal identities remain in separate authentication realms. Company-isolation and role permissions are enforced by the service boundary.</p><p>{serviceMode === 'mock' ? 'This demonstration stores fabricated data on this device. Do not enter real customer, pricing, credential or infrastructure information.' : 'Production data rights, retention and access requests must use the approved backend process.'}</p><p>Operational records, audit history and signed documents cannot be erased from this settings page.</p></SettingsPanel>; }
function AboutSettings({ serviceMode }) { return <SettingsPanel title="Rhomberg Connect"><img className="settings-about-logo" src="assets/images/rhomberg-connect-logo-full-dark.png" alt="Rhomberg Connect" /><dl className="settings-session"><div><dt>Version</dt><dd>5.0.0</dd></div><div><dt>Environment</dt><dd>{serviceMode === 'mock' ? 'Fabricated demonstration' : 'Private company service'}</dd></div><div><dt>Shared platform</dt><dd>Customer, Sales, Technical and Operations</dd></div></dl></SettingsPanel>; }
