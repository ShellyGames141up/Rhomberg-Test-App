...
const accountRoles = account => new Set([account?.role, ...(account?.labRoles || [])]);
const canOperateMethod = (account, methodId) => {
  const roles = accountRoles(account);
  return methodId === 'temperature_comparison'
    ? roles.has('laboratory_temperature_technician')
...
const WORKFLOW_STEPS = [
  ...
  { label: 'Stabilise', statuses: ['thermal_stabilisation'] },
  { label: 'Inspect & book', statuses: ['inspection_pending', 'inspection_failed', 'booked_in'] },
  { label: 'Calibrate', statuses: ['worksheet_ready', 'calibration_in_progress', 'calibration_on_hold', 'calculation_review_required', 'management_changes_required'] },
  { label: 'Label & dispatch', statuses: ['calibration_completed', 'labelling_pending', 'labelling_completed', 'bom_signoff_pending'] },
  { label: 'Certificate', statuses: ['released_to_dispatch', 'draft_certificate_ready', 'management_review', 'approved_for_signature', 'awaiting_signed_certificate', 'signed_certificate_uploaded'] },
  { label: 'Released', statuses: ['certificate_released', 'completed', 'archived'] },
];

const defaultForm = (order, unit, item, options) => {
  const temperature = isTemperature(unit, item);
  const methodId = temperature ? 'temperature_comparison' : 'pressure_dwt_700_bar';
...
export function LaboratoryDashboard({ account, orders, laboratoryActions, laboratoryOptions = {}, serviceMode, onRecordsChanged, focusRecordId }) {
  ...
  const setForm = (order, unit, item, key, value) => setForms(current => {
    const base = current[unit.id] || defaultForm(order, unit, item, laboratoryOptions);
    const patch = typeof key === 'object' ? key : { [key]: value };
    return { ...current, [unit.id]: { ...base, ...patch } };
  });
  const setPoint = (order, unit, item, pointIndex, key, value) => {
    setForms(current => {
      const base = current[unit.id] || defaultForm(order, unit, item, laboratoryOptions);
      const testPoints = base.testPoints.map((point, index) => index === pointIndex ? { ...point, [key]: value } : point);
      return { ...current, [unit.id]: { ...base, testPoints } };
    });
  };
  ...
  return (
    <section className="app-screen operations-desktop laboratory-screen lab-control-centre" aria-labelledby="laboratory-title">
      ...
      <nav className="lab-section-tabs" aria-label="Laboratory workspace sections">
        {[['queue', 'Laboratory queue'], ['management', 'Management review'], ['certificates', 'Certificates'], ['standards', 'Reference standards'], ['monthly', 'Monthly tracker']].map(([id, label]) => <button key={id} type="button" className={tab === id ? 'is-active' : ''} aria-current={tab === id ? 'page' : undefined} onClick={() => { setTab(id); if (id === 'management') setStatus('management'); }}>{label}</button>)}
      </nav>
...
function StagePanel({ order, unit, form, set, setPoint, options, allowedStandards, actions, call, account, disabled }) {
  ...
  const selectMethod = methodId => {
    const branchId = unit.labWork?.branchId || form.branchId;
    const standard = (options.referenceStandards || []).find(candidate => candidate.branchId === branchId && candidate.approvedMethods.includes(methodId));
    set({ methodId, standardIds: standard ? [standard.id] : [] });
  };
  ...
  if (status === 'booked_in') return <ActionCard title="Structured booking-in" description="Order data is reused; complete only the physical instrument and method details."><FormGrid><Input label="Instrument description" value={form.instrumentDescription} onChange={value => set('instrumentDescription', value)} /><Input label="Manufacturer" value={form.manufacturer} onChange={value => set('manufacturer', value)} /><Input label="Model" value={form.model} onChange={value => set('model', value)} /><Input label="Serial number" value={form.serialNumber} onChange={value => set('serialNumber', value)} /><Input label="Range minimum" type="number" value={form.rangeMinimum} onChange={value => set('rangeMinimum', value)} /><Input label="Range maximum" type="number" value={form.rangeMaximum} onChange={value => set('rangeMaximum', value)} /><Input label="Unit" value={form.unit} onChange={value => set('unit', value)} /><Input label="Resolution" type="number" value={form.resolution} onChange={value => set('resolution', value)} /><Select label="Method" value={form.methodId} onChange={selectMethod} options={options.methods || []} /></FormGrid><Primary disabled={disabled || !can(PERMISSIONS.BOOK_IN_LAB_UNIT)} onClick={() => call('book', () => actions.bookIn(order.id, unit.id, form), 'Job and certificate numbers assigned.')}>Book In Physical Unit</Primary></ActionCard>;
  if (['worksheet_ready', 'calibration_in_progress', 'calibration_on_hold', 'management_changes_required'].includes(status)) return <WorksheetCard form={form} set={set} setPoint={setPoint} onMethodChange={selectMethod} standards={allowedStandards} unit={unit} order={order} actions={actions} call={call} account={account} disabled={disabled} />;
  if (status === 'calculation_review_required') {
    const reviewed = unit.labWork.formulaValidationReview?.status === 'accepted_for_mock_demo';
    const technicianCanComplete = can(PERMISSIONS.COMPLETE_LAB_CALIBRATION) && canOperateMethod(account, unit.labWork.worksheet?.methodId || form.methodId);
    return <ActionCard title={reviewed ? 'Technician completion required' : 'Management calculation review required'} description={reviewed ? 'Management review is recorded. An authorised technician can now confirm and complete the calibration.' : 'The structured calculation is locked. Management must review the calculation before the technician can complete calibration.'}>
      <CalculationSummary calculation={unit.labWork.calculation} />
      {can(PERMISSIONS.REVIEW_RAW_LAB_DATA) && <Secondary disabled={disabled} onClick={() => call('review-pdf', () => actions.generateReviewPdf(order.id, unit.id), 'Internal review PDF generated.', true)}>Download Internal Review PDF</Secondary>}
      {!reviewed && can(PERMISSIONS.APPROVE_CALCULATION_REVIEW) && <><Textarea label="Management review reason" value={form.formulaReason} onChange={value => set('formulaReason', value)} /><Secondary disabled={disabled || form.formulaReason.trim().length < 12} onClick={() => call('formula-review', () => actions.approveFormulaValidation(order.id, unit.id, { confirmed: true, reason: form.formulaReason }), 'Management review recorded. The technician can now complete calibration.')}>1. Record Management Review</Secondary></>}
      {!reviewed && !can(PERMISSIONS.APPROVE_CALCULATION_REVIEW) && <RoleHandoff role="Laboratory Manager" message="The calculation is ready. Ask Laboratory Management to record the technical review." />}
      {reviewed && technicianCanComplete && <><label className="lab-check"><input type="checkbox" checked={form.technicianConfirmed} onChange={event => set('technicianConfirmed', event.target.checked)} /><span>I confirm the readings and repeatability checks are complete</span></label><Primary disabled={disabled || !form.technicianConfirmed} onClick={() => call('complete-calibration', () => actions.completeCalibration(order.id, unit.id, { technicianConfirmed: true, resultSummary: form.calibrationResult }), 'Calibration completed.')}>2. Complete Calibration</Primary></>}
      {reviewed && !technicianCanComplete && <RoleHandoff role={`${methodDiscipline(unit.labWork.worksheet?.methodId)} Technician`} message="Management review is complete. The authorised technician must perform the final calibration confirmation." />}
    </ActionCard>;
  }
...
function WorksheetCard({ form, set, setPoint, onMethodChange, standards, unit, order, actions, call, account, disabled }) {
  ...
  return <ActionCard title={`${humanise(form.methodId)} worksheet`} description="Raw inputs remain distinct from calculated and management-only fields. Readings accept comma-separated numeric values.">
    ...
    {saved && unit.labWork.status === 'calibration_on_hold' && technicianCanOperate && <Primary disabled={disabled || !accountCan(account, PERMISSIONS.START_LAB_CALIBRATION)} onClick={() => call('resume', () => actions.startCalibration(order.id, unit.id, { note: 'Resumed after controlled hold' }), 'Calibration resumed.')}>Resume Calibration</Primary>}
    {unit.labWork.status === 'calibration_in_progress' && technicianCanOperate && <><Textarea label="Reason to put calibration on hold" value={form.holdReason} onChange={value => set('holdReason', value)} /><Secondary disabled={disabled || form.holdReason.trim().length < 8 || !accountCan(account, PERMISSIONS.START_LAB_CALIBRATION)} onClick={() => call('hold', () => actions.holdCalibration(order.id, unit.id, { reason: form.holdReason }), 'Calibration placed on controlled hold.')}>Put Calibration on Hold</Secondary></>}
    {saved && unit.labWork.status === 'calibration_in_progress' && technicianCanOperate && <Primary disabled={disabled} onClick={() => call('calculate', () => actions.calculate(order.id, unit.id), 'Calculations completed; management review is now required.')}>3. Calculate and Send for Review</Primary>}
  </ActionCard>;
}

function CertificateWorkflow({ status, form, set, unit, order, actions, call, account, disabled }) {
  const can = permission => accountCan(account, permission);
  if (status === 'released_to_dispatch') return <ActionCard title="Certificate compilation" description="The physical unit is with Dispatch; certificate work remains active.">{can(PERMISSIONS.REVIEW_RAW_LAB_DATA) && <Secondary disabled={disabled} onClick={() => call('review-pdf', () => actions.generateReviewPdf(order.id, unit.id), 'Internal review PDF generated.', true)}>Download Review PDF</Secondary>}{can(PERMISSIONS.GENERATE_DRAFT_CERTIFICATE) ? <Primary disabled={disabled} onClick={() => call('draft', () => actions.generateDraftCertificate(order.id, unit.id), 'Draft certificate generated.', true)}>1. Generate Draft Certificate</Primary> : <RoleHandoff role="Laboratory Manager" message="The unit is with Dispatch. Laboratory Management must generate the controlled certificate draft." />}</ActionCard>;
  if (status === 'draft_certificate_ready') return <ActionCard title="Draft certificate ready" description="The immutable draft is linked to the raw-data and calculation versions."><Textarea label="Review submission comment" value={form.reviewComment} onChange={value => set('reviewComment', value)} />{can(PERMISSIONS.REVIEW_RAW_LAB_DATA) ? <Primary disabled={disabled} onClick={() => call('submit-review', () => actions.submitCertificateForReview(order.id, unit.id, { comment: form.reviewComment }), 'Certificate submitted for management review.')}>2. Submit for Management Review</Primary> : <RoleHandoff role="Laboratory Manager" message="A Laboratory Manager must submit the draft into controlled review." />}</ActionCard>;
  ...
  if (status === 'approved_for_signature') return <ActionCard title="Download certificate for signing" description="Download and sign only with Rhomberg's approved external PDF signing process.">{can(PERMISSIONS.GENERATE_DRAFT_CERTIFICATE) ? <><ol className="lab-upload-steps"><li>Download the final unsigned PDF.</li><li>Sign it outside this application using the approved process.</li><li>Return here to upload the signed PDF.</li></ol><Primary disabled={disabled} onClick={() => call('unsigned', () => actions.generateUnsignedCertificate(order.id, unit.id), 'Unsigned certificate downloaded. Sign it externally, then return to upload it.', true)}>3. Download Final Unsigned PDF</Primary></> : <RoleHandoff role="Technical Signatory" message="An authorised signatory must generate the controlled unsigned certificate." />}</ActionCard>;
  if (status === 'awaiting_signed_certificate') return <ActionCard title="Upload the signed certificate" description="Choose the signed PDF produced by the approved external signing process. The file is hashed and stored unchanged.">{can(PERMISSIONS.UPLOAD_SIGNED_CERTIFICATE) ? <><ol className="lab-upload-steps"><li>Confirm the certificate number and issue date.</li><li>Select the externally signed PDF.</li><li>Upload it to continue to final release.</li></ol><FormGrid><Input label="Certificate number" value={unit.certificateNumber || ''} readOnly /><Input label="Issue date" type="date" value={form.signedIssueDate} onChange={value => set('signedIssueDate', value)} /><label><span>Signed PDF</span><input type="file" accept="application/pdf,.pdf" onChange={event => set('signedFile', event.target.files?.[0] || null)} /><small>{form.signedFile ? `Selected: ${form.signedFile.name}` : 'PDF required'}</small></label></FormGrid><Primary disabled={disabled || !form.signedFile} onClick={() => call('signed-upload', () => actions.uploadSignedCertificate(order.id, unit.id, { file: form.signedFile, certificateNumber: unit.certificateNumber, issueDate: form.signedIssueDate, reason: form.signedReason }), 'Signed PDF uploaded, verified and hashed.')}>4. Upload Signed PDF</Primary></> : <RoleHandoff role="Technical Signatory" message="The signed PDF is ready to be returned by an authorised certificate user." />}</ActionCard>;
  if (status === 'signed_certificate_uploaded') return <ActionCard title="Final certificate release" description="Select the management-approved recipient rule."><Select label="Recipients" value={form.recipientRule} onChange={value => set('recipientRule', value)} options={[{ id: 'representative_only', label: 'Assigned representative only' }, { id: 'customer_and_representative', label: 'Customer and assigned representative' }]} />{can(PERMISSIONS.RELEASE_CERTIFICATE) ? <Primary disabled={disabled} onClick={() => call('release-certificate', () => actions.releaseCertificate(order.id, unit.id, { recipientRule: form.recipientRule }), 'Certificate released and recipients notified.')}>5. Release Certificate</Primary> : <RoleHandoff role="Laboratory Manager" message="The signed PDF is secure. Laboratory Management must perform the final release." />}</ActionCard>;
...
function LaboratoryProgress({ status }) {
  const current = Math.max(0, WORKFLOW_STEPS.findIndex(step => step.statuses.includes(status)));
  return <ol className="lab-progress" aria-label="Laboratory workflow progress">{WORKFLOW_STEPS.map((step, index) => <li key={step.label} className={index < current ? 'is-complete' : index === current ? 'is-current' : ''} aria-current={index === current ? 'step' : undefined}><span>{index < current ? '✓' : index + 1}</span><strong>{step.label}</strong></li>)}</ol>;
}
function RoleHandoff({ role, message }) { return <div className="lab-handoff-note" role="status"><strong>Next: {role}</strong><span>{message}</span></div>; }
