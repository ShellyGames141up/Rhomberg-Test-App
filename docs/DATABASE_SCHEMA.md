...
Migration `016_conditional_product_configuration.sql` normalises persisted catalogue schemas so the custom-range description is required only when `Custom range - sales review` is selected. Server-side product validation also enforces every conditional `showWhen` rule and validates multi-choice values individually; hidden configuration fields cannot block an otherwise complete RFQ or Representative-loaded order.

Migration `017_restore_customer_lifecycle_functions.sql` repairs staging databases that recorded the onboarding migrations before the final approved customer-registration, Dedicated Representative RFQ-routing and Administrator soft-delete functions were present. The runtime-grant script now checks the explicit function allowlist before revoking existing privileges, resolves signatures from PostgreSQL's catalogue and verifies every execute grant before deployment can continue.
