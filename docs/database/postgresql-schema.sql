...
CREATE TYPE app.enquiry_status AS ENUM ('draft', 'rfq_submitted', 'under_review', 'quotation_sent', 'converted_to_order', 'closed', 'cancelled');
CREATE TYPE app.order_status AS ENUM ('po_received', 'scheduled', 'in_production', 'quality_check', 'ready', 'dispatched', 'completed', 'on_hold', 'cancelled');
...
CREATE TABLE app.enquiries (
  ...
  status app.enquiry_status NOT NULL DEFAULT 'rfq_submitted',
...
CREATE TABLE app.orders (
  ...
  status app.order_status NOT NULL DEFAULT 'po_received',
  po_number text,
  promised_date date,
...
CREATE TABLE app.tracking_events (
  ...
  customer_visible_note text NOT NULL,
  actor_user_id uuid NOT NULL REFERENCES app.users(id),
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT tracking_event_parent CHECK (num_nonnulls(enquiry_id, order_id) = 1)
);
...
CREATE INDEX tracking_events_enquiry_idx ON app.tracking_events (enquiry_id, created_at);
CREATE INDEX tracking_events_order_idx ON app.tracking_events (order_id, created_at);
...
AS $$
  SELECT CASE
    WHEN app.current_user_role() IN ('expeditor', 'buyer', 'manager', 'administrator') THEN true
...
CREATE POLICY tracking_events_authorised_scope ON app.tracking_events
  USING (
...
-- In particular, customers must not receive UPDATE/DELETE rights on tracking events,
