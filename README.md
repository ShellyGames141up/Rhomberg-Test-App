...
Staging 5.2.22 preparation adds a dedicated, audited **Records** workspace for Administrator and Planning. Administrator may remove active RFQs or orders; Planning is restricted to orders still in its own queue. Removal is soft, requires a reason, preserves documents and immutable audit history, and respects legal holds. The account-deletion client now sends the required JSON request body. This release requires migration 026 and matching runtime grants. Deploy the matching API and frontend together; local validation is not RHOMAPP acceptance.
...
remains read-only and database-authorised. This correction is not yet deployed.
...
Product Completion Pass 2 expands this foundation through migrations 004–012 and the existing service layer. The current generated audit covers **135 active frontend API contracts and 143 backend routes, with zero missing contracts and zero transport incompatibilities**. Persisted workflows now include RFQ/order progression, Technical Support, representative-loaded orders, Planning, Expediting, QA, Dispatch, the approved simplified Laboratory Manager certificate workflow, authenticated documents, administration lifecycle, governance/archive, client visits, settings/personalisation and management projections. Migrations 013–017 add first-login password enforcement, customer onboarding with a persistent Dedicated Representative relationship, Administrator soft deletion, consistent conditional product-configuration validation, and an idempotent repair for staging databases whose recorded onboarding migrations predated the final approved lifecycle functions. Runtime database grants now validate the approved function surface before revoking access and verify every catalogue-resolved execute permission. This is internal-test readiness evidence—not deployment, production or real-data approval. See [the completion report](docs/PRODUCT_COMPLETION_PASS_2_REPORT.md), [completion matrix](docs/INTERNAL_TEST_PRODUCT_COMPLETION_MATRIX.md) and [generated contract audit](docs/API_MODE_CONTRACT_AUDIT.md).

The controlled customer-onboarding extension adds server-backed **Create Account**, migration 014 and first-RFQ Dedicated Representative selection. The representative belongs to the company/customer relationship: the first choice is restricted to active representatives for the company's area, later RFQs use it automatically, and Administrator reassignment is reasoned and audited. Every RFQ separately snapshots its assigned representative, so changing the relationship never rewrites historical ownership. External identity verification and production anti-abuse controls remain prerequisites for real customer onboarding.

Administrator-assigned customer relationships are projected directly into the RFQ screen as a read-only, automatically selected Dedicated Representative. Migration 016 and the shared server validator now apply required-field checks only to product options that are actually visible for the chosen configuration; a standard catalogue range no longer incorrectly demands a hidden custom-range description, while genuine custom-range and other conditional requirements remain enforced.

Administrator-created customer and employee accounts can sign in immediately with the supplied temporary password. The server then restricts that session to the mandatory first-login password-change screen; an Administrator reset is not part of normal onboarding. Administrators can also soft-delete non-Administrator accounts with a recorded reason. Deletion revokes access and active assignments while preserving companies, RFQs, orders, documents and immutable audit history. Migration 015 supplies the server-authoritative deletion operation.
...
adds migration 018: assigned roles automatically supply their default permissions,
multiple roles combine access, removed roles lose their exclusive defaults, and
audited per-account additions/restrictions remain explicit. The Administrator
editor displays real effective permissions and confirms sensitive changes on the
server. Deployment requires migration 018 and the updated runtime grants; this
source correction does not itself update RHOMAPP.
