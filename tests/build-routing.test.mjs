...
const application = previewContextForPath('/Rhomberg-Test-App/app/');
assert.equal(application.id, PREVIEW_IDS.APPLICATION);
...
assert.ok(applicationDocument.includes('<meta name="rhomberg-preview" content="application">'));
assert.ok(applicationDocument.includes('<base href="../">'));
...
for (const stagedPath of ["path.join(root, 'app')", "path.join(root, 'preview')", "path.join(root, 'demo')"]) {
