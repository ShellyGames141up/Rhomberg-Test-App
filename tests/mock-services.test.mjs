...
await apiServices.laboratory.uploadCertificate('order-lab-api', 'unit-lab-api', { certificateNumber: 'CAL-API-001', issueDate: '2026-08-04', serialNumber: 'FAB-UNIT-001', certificationType: 'SANAS', confirmAssociation: true, file: apiSignedCertificate });
...
const certificateApiRequest = apiRequests.find(request => request.path.endsWith('/laboratory/orders/order-lab-api/units/unit-lab-api/certificate'));
assert.ok(certificateApiRequest.options.body instanceof FormData, 'Laboratory certificates must use controlled multipart upload');
assert.ok(certificateApiRequest.options.headers['Idempotency-Key'], 'certificate uploads must carry an idempotency key');
