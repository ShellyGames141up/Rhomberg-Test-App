...
for (const definition of PREVIEW_DEFINITIONS) {
  ...
  assert.ok(page.includes('app.js?v=38'), `${definition.id} must request the current application bundle`);
}
const rootPage = readFileSync(path.resolve('index.html'), 'utf8');
const serviceWorker = readFileSync(path.resolve('sw.js'), 'utf8');
assert.ok(rootPage.includes('app.js?v=38'), 'Preview Centre must request the current application bundle');
assert.ok(serviceWorker.includes("'./app.js?v=38'"), 'service worker must cache the same application bundle version');
