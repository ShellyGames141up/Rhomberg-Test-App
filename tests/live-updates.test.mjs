...
test('fifteen-minute polling skips unchanged data, hidden/offline views and preserves unsaved work', async () => {
  const h = harness();
  assert.equal(h.scheduled().delay, 900000);
  await h.polling.refresh(); assert.equal(h.applications(), 1);
  assert.equal(h.scheduled().delay, 900000, 'successful polling must not shorten the interval');
...
test('poll failures retain data and back off; successful recovery resets interval', async () => {
  ...
  await h.polling.refresh(); assert.equal(h.scheduled().delay, 1200000); assert.equal(h.applications(), 0);
  await h.polling.refresh(); assert.equal(h.scheduled().delay, 1200000);
  await h.polling.refresh(); assert.equal(h.scheduled().delay, 1200000);
  failed = false; await h.polling.refresh(); assert.equal(h.scheduled().delay, 900000); assert.equal(h.applications(), 1);
  h.polling.stop();
});
